...
var counter = 0, // 一開始要顯示的圖，0 的話就是顯示第一張
    ...
    items = slide1.querySelectorAll('img'), // 抓取所有 img
    itemsCount = items.length, // 圖片總數 
    prevBtn = document.createElement('a'), // 上一張按鈕
    nextBtn = document.createElement('a'), // 下一張按鈕
    timer = 4000, // 4 秒換圖
...
prevBtn.classList.add('prev'); // 幫上一張按鈕加 class＝"prev" 給 CSS 指定樣式用
nextBtn.classList.add('next'); // 幫下一張按鈕加 class＝"next" 給 CSS 指定樣式用
slide1.appendChild(prevBtn); // 將按鈕加到 #slide1 裡
slide1.appendChild(nextBtn);
...
// 滑鼠移到 #slide1r 上方時，停止循環計時
slide1.addEventListener('mouseover', function(){
    interval = clearInterval(interval);
});

// 滑鼠離開 #slide1r 時，重新開始循環計時
...
var counter = 0, // 一開始要顯示的圖，0 的話就是顯示第一張
    ...
    items = slide2.querySelectorAll('img'), // 抓取所有 img
    itemsCount = items.length, // 圖片總數 
    prevBtn = document.createElement('a'), // 上一張按鈕
    nextBtn = document.createElement('a'), // 下一張按鈕
    timer = 4000, // 4 秒換圖
...
prevBtn.classList.add('prev'); // 幫上一張按鈕加 class＝"prev" 給 CSS 指定樣式用
nextBtn.classList.add('next'); // 幫下一張按鈕加 class＝"next" 給 CSS 指定樣式用
slide2.appendChild(prevBtn); // 將按鈕加到 #slide2 裡
slide2.appendChild(nextBtn);
...
// 滑鼠移到 #slide2r 上方時，停止循環計時
slide2.addEventListener('mouseover', function(){
    interval = clearInterval(interval);
});

// 滑鼠離開 #slide2r 時，重新開始循環計時
